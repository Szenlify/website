...
export default function HomeView({ dict, locale, children }: HomeViewProps) {
    ...
    return (
        <div className={user ? "pb-24 sm:pb-0" : undefined}>
            {user && (
                /* Główny kontener z animacją pojawiania się od dołu */
                <div className="fixed bottom-[max(1rem,env(safe-area-inset-bottom))] right-4 sm:bottom-6 sm:right-6 z-50 animate-in fade-in slide-in-from-bottom-5 duration-700 ease-out fill-mode-forwards">
                    <Link
                        href={getLocalizedHref("/dashboard/reviews", locale)}
                        className="group relative inline-flex items-center justify-center cursor-pointer rounded-full focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-indigo-300"
                    >
                        ...
                        <div className="relative min-h-12 inline-flex items-center gap-2.5 px-4 py-2.5 rounded-full bg-[#1f0226] border border-white/10 text-zinc-300 text-xs sm:text-sm font-medium shadow-2xl transition-all duration-300 group-hover:border-white/25 group-hover:text-white group-active:scale-95">
