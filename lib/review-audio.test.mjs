...
test('OPENAI_VOICE_GAIN compensates Nova with higher gain than Alloy', () => {
  assert.ok(OPENAI_VOICE_GAIN.nova >= 1.7, 'Nova should be boosted by at least 1.7x to compensate for quiet RMS');
  assert.ok(OPENAI_VOICE_GAIN.alloy >= 1.2, 'Alloy should have balanced presence');
  assert.ok(OPENAI_VOICE_GAIN.nova > OPENAI_VOICE_GAIN.alloy, 'Nova should have higher boost than Alloy');
});

test('getOpenAi with alloy calls geminiProxy to count characters, and reuses local memory without recharging', async () => {
  const original = globalThis.fetch;
  const cache = new ReviewAudioCache();
  ...
  globalThis.fetch = async (url, options) => {
    ...
    if (urlStr.includes('geminiProxy')) {
      proxyCallCount++;
      const body = JSON.parse(options.body);
      assert.equal(body.voiceId, 'alloy');
      const headers = new Headers();
      headers.set('X-Lectoro-TTS-Used', '42');
      return new Response(new Blob(['mp3alloy'], { type: 'audio/mpeg' }), { headers });
    }
    return new Response(null, { status: 404 });
  };
  ...
  try {
    ...
    const audioUrl1 = await cache.getOpenAi('Good morning', 'en', 'alloy', 'mock-token');
    ...
    const audioUrl2 = await cache.getOpenAi('Good morning', 'en', 'alloy', 'mock-token');
